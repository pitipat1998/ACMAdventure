...
public class MST {
	...
	static TreeSet<int[]>[] vtxs = (TreeSet<int[]>[]) new TreeSet[(int)(2e5)];
	static long sum = 0;
	
	
	private static int root(int u) {
		if(nodes[u] != u) nodes[u] = root(nodes[u]);
		return nodes[u];
	}
	...
	public static void initialize() {
		...
		for(int n=0; n<N; n++) {
			nodes[n] = n;
			sizes[n] = 1;
			vtxs[n] = new TreeSet<>((int[] a, int[] b) -> b[1] - a[1]);
		}
	}
	...
	public static void main(String[] args) {
		...
		for(int m=0; m<M; m++) {
			...
			es[m] = e;
			vtxs[u].add(new int[] {v, w});
			vtxs[v].add(new int[] {u, w});
		}
